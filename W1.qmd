---
title: "Workshop 1: Welcome to Quarto!"
subtitle: |
...
Use 1 to 4 hashtags to make a **header.** More hashtags = *smaller* header! You'll need to use headers to organize your homework assignments for this class.
...
To include R code in your document, you'll need to distinguish it from text. To do this, create a "chunk" using the back-tick ` character (same key as the \~ key on most keyboards).
...
So you type in a .qmd file, you produce a .pdf file, and a bunch of weird files are generated and erased during the rendering process. We're about to start using images and datasets too. You'll want to store all this stuff in the same folder: this will be your *working directory.*
...
#### Creating a Table
This might be handy for displaying output like model coefficients and p-values towards the end of the course. It's compatible with in-line code, too.

[ANSWER]

Notice the alignment of the columns is controlled by the placement of the colon (:)

You can also create a table using the `kable()` function from the `knitr` package:

[ANSWER]

# TLDR

I suggest you put your own TLDR here. There's no coffee date for the workshop, but summarizing a good way to make sure the big ideas stick.
