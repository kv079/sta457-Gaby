...
# --- Plotting
plot.wine() # plot
mtext("Polynomial Regression: p = 1") # Add a subtitle indicating model
* lines(, col = "blue") # add trend estimate in blue
```
...
* plot.ts(,
        ...
        xaxt = 'n')
axis.wine()
mtext("Polynomial Regression: p = 1")
abline(h = 0, lty = 2) # (draw a dashed line at y = 0)
```
...
We suspect there is a seasonal component of period $d = 12$. Let's model this, and plot it over our exponential smoother residuals. Are there other periods that might be relevant?
...
Let's try using the `season()` function from ITSMR. Again, we choose $d = 12$.
...
* plot.ts(,
        main = "Happy Wine | MA Smoother Residuals",
        ylab = "Residual Value",
        xaxt = 'n')
...
Basically, this describes the correlation between any two points in the series, as a function of the time-distance between those two points. Noise, by definition, has no such correlation across time.

We want our residuals $\hat Y_t$ to be *noisy.* We want our pen's ink to be *inky.*
