...
# --- Plotting
plot.wine()
mtext("Polynomial Regression: p = 1") # Adds a subtitle
lines(m.pr, col = "blue")
```

Now we need to compute and plot the residuals: $(x_t - \hat m_t)$. Think: ink tube + spring
...
s.s1 <- season(rm.ma, d = 12)   # Seasonal Component
...
```{r}
y.hr <- rm.exp - s.hr          # Harmonic Regression Residuals
y.s1 <- rm.ma - s.s1           # S1 Residuals
```
...
Basically, it describes the correlation between any two points in the series, as a function of the time-distance between those two points. Noise, by definition, has no such correlation across time.
